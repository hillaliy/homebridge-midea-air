...
class Utils {
    ...
    static getSignPassword(loginId, password) {
        if (loginId != '' && password != '') {
            const pw = crypto_1.default.createHash("sha256").update(password).digest("hex");
            return crypto_1.default.createHash("sha256").update(loginId + pw + Constants_1.default.AppKey).digest("hex");
        }
        else {
            return '';
        }
    ...
    static generateDataKey(accessToken) {
        if (accessToken != '') {
            const md5AppKey = crypto_1.default.createHash("md5").update(Constants_1.default.AppKey).digest("hex");
