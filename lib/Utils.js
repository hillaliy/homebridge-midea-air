...
class Utils {
    ...
    ;
    static getSign(path, form, appKey) {
        if (path !== '' && form) {
            let postfix = `/v1${path}`;
    ...
    ;
    static getSignPassword(loginId, password, appKey) {
        if (loginId !== '' && password !== '') {
    ...
    ;
    static generateDataKey(accessToken, appKey) {
        if (accessToken != '') {
    ...
    ;
    static encryptIAMPassword(loginId, password, appKey) {
        const passwordHash = crypto_1.default
            .createHash('md5')
            .update(password)
            .digest();
        const password2ndHash = crypto_1.default
            .createHash('md5')
            .update(passwordHash)
            .digest();
        return crypto_1.default
...
Utils.randomString = Math.floor(new Date().getTime() / 1000).toString();
Utils.pushToken = crypto_1.default.randomBytes(120).toString('base64');
Utils.reqId = crypto_1.default.randomBytes(16).toString('hex');
;
